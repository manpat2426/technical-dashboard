...
- **The decision columns are in the data feed but not on the page yet.** As of 2026-08-02 `dashboard_data.json` carries `Decision`, `Decision_1W`, `Decision_2W`, `Decision_3W`, and `Decision_1M` for every symbol, but `index.html` was deliberately left untouched — it doesn't read or render those fields. Nothing on the live site looks different. Adding the columns (extending the shared color scale to the six states with Neutral reading yellow/gray, and fitting them into the sticky-column and mobile layouts) is a separate follow-up.
...
- **Pending: render the decision columns on the dashboard.** The decision data exists everywhere it needs to — Airtable, `history_log.jsonl`, and `dashboard_data.json` — but `index.html` doesn't display it yet (see Section 7). That front-end work is the intended next step.
