...
**Not part of the daily pipeline** (each file says so in its own docstring): `smoke_test.py`, `scoring_check.py`, `checkpoint_check.py`, `momentum_check.py`. These are manual debugging scripts kept around for future troubleshooting (EODHD connectivity, score sanity-checking, checkpoint retrieval, and Improving/Deteriorating firing-rate checks, respectively). They're safe to ignore for normal operation. All four are **committed** — `checkpoint_check.py` and `momentum_check.py` had existed only in a local working copy until 2026-08-23, which meant a fresh clone silently lacked two of the four scripts this section claimed it had. Nothing in the pipeline imports them, but they read live Config/EODHD data when run, so they need the same `.env` setup as `main.py`.
...
- **Windows/OneDrive: make sure you're in the *real* working copy.** The local clone lives under a OneDrive-backed `Desktop`. Because Windows can redirect `Desktop` into OneDrive, the same project can appear at two paths — `...\Desktop\...\technical-dashboard` and `...\OneDrive\Desktop\...\technical-dashboard`. If that redirection is ever turned off or changes, the **non**-OneDrive path is left behind as a hollow shell: the folder still exists but is empty, with no `.git`, so git reports *"fatal: not a git repository"* and the files look deleted. **They are not** — the real clone, `.git`, and the un-tracked `.env` are all intact at the OneDrive path. Confirm with `git -C <path> rev-parse HEAD` before concluding anything is lost, and delete the empty shell so it can't be opened by mistake. Nothing about the live pipeline is affected either way: Actions runs from GitHub and Pages serves from GitHub, so a broken local copy only blocks local work.
...
I compared every specific number in `METHODOLOGY.md` against the live Config table (21 real parameters) and `scoring.py`. Result: **everything checked out exactly**, with two things flagged — the first since resolved, the second still open:

1. **Resolved: not every threshold lives in Config, and METHODOLOGY.md now says so.** All **Trend** thresholds (`PRICE60_HIGH`=1.05, `PRICE60_LOW`=0.95, `PRICE120_HIGH`=1.10, `PRICE120_LOW`=0.90) and all **Volatility**/indicator-period parameters (`ATR_Period`=20, `ATR_Baseline_Period`=90, `BB_Period`=20, `BB_BBW_Period`=125, `RSI_Period`=14, `ROC_Period`=20, `MACD_Fast`=12, `MACD_Slow`=26, `MACD_Signal`=9) are genuinely in Config and match METHODOLOGY.md exactly. But the **Momentum regime cutoffs** (RSI ≥60/≥50/≤40/<50, the MACD/ROC sign checks) and the **Improving/Deteriorating tuning constants** (the ~5-trading-day lookback, ±3 RSI points, ±0.02 ROC) are hardcoded in `scoring.py`, and the whole **decision layer** (§5) is hardcoded in `decision.py`'s constants block — none of them read from Config. METHODOLOGY.md originally claimed Config was the authoritative source for everything; its opening note now names both code-resident exceptions explicitly, and §1's "every threshold lives in Config" line has been reworded to the accurate principle — *every parameter lives in exactly one declared place, most in Config and the rest in labeled constants blocks; nothing is buried in the logic*. The alternative fix — actually moving those constants into Config — was **not** done and remains available if runtime tunability for them is ever wanted; note it would mean a Config edit could silently change regime classification between runs, which is precisely why the decision layer's constants were deliberately put in code in the first place.
