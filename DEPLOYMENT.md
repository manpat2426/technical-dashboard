...
- **`numpy` is an undeclared direct dependency.** `decision.py` does `import numpy as np`, but `requirements.txt` lists only `requests`, `python-dotenv`, and `pandas`. This does not currently fail, because pandas depends on numpy and so always installs it transitively — but relying on a transitive dependency for a direct import is fragile: a future pandas that changed how it vendors numpy would break the run at *import time*, before the decision guard's protection applies. The fix is trivial (add a `numpy` line to `requirements.txt`) and worth doing next time the file is touched.
